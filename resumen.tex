...
Comenzamos probando desde n>1. Los n que queremos probar deben ser mayores que un m dado. Es importante notar que como los casos anteriores no nos importan, los asumimos como verdaderos porque como no hablamos de ellos las implicancias quedan falsas. \\
Ej.: Si me piden probar desde $n>3$ entonces tengo que considerar todos los $n>m$ donde m es 3 e indica que los primeros 3 casos no nos importan. Por lo tanto las implicaciones falsas terminan dando verdadero. \\
...
\textbf{Caso Base}: P(n) se cumple con $m>1$ \\
\textbf{Paso Inductivo}: Sea $ k \in \nat>=m $
\begin{itemize}
    \item \textbf{HI}: P(k)
